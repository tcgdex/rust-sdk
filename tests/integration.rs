...
async fn test_get_card() {
    ...
    let card = tcgdex.card.get("base1-4").await.unwrap();
...
async fn test_card_list() {
    ...
    let cards = tcgdex.card.list(Some(&query)).await.unwrap();
...
async fn test_set() {
    ...
    let set = tcgdex.set.get("base1").await.unwrap();
...
async fn test_serie() {
    ...
    let serie = tcgdex.serie.get("base").await.unwrap();
...
async fn test_different_languages() {
    ...
    let card_en = tcgdex_en.card.get("base1-4").await.unwrap();
    assert_eq!(card_en.name, "Charizard");

    // Test French
    let tcgdex_fr = TCGdex::new(Language::FR);
    let card_fr = tcgdex_fr.card.get("base1-4").await.unwrap();
    assert_eq!(card_fr.name, "Dracaufeu");

    // Test German
    let tcgdex_de = TCGdex::new(Language::DE);
    let card_de = tcgdex_de.card.get("base1-4").await.unwrap();
...
async fn test_string_endpoints() {
    ...
    // Test type endpoint
    let types_list = tcgdex.type_.list(None).await.unwrap();
    assert!(!types_list.is_empty());

    // We expect each StringList to contain various types including "Fire"
    let has_fire = types_list
        .iter()
        .any(|string_list| string_list.contains("Fire"));
...
async fn test_complex_query() {
    ...
    let card_resumes = tcgdex.card.list(Some(&query)).await.unwrap();
...
async fn test_card_image_url() {
    ...
    // but we could use card.get_image(&tcgdex, Quality::HIGH, Extension::PNG).await
}
